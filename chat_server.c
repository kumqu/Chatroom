		...
		for (tmp_i = sockfd; tmp_i <= max_fd; tmp_i++)
		{
			...
			if (FD_ISSET(tmp_i, &read_fds))
			{
				if (tmp_i == sockfd)
				{
					...
					clientinfo[newfd].online = 1;
					strcmp(clientinfo[newfd].username,"");
				...
				{/*get data from the client*/
                    ...
                    if(clientinfo[newfd].first == 1)                          //判断是否是第一次登陆
					{
                        memset(clientinfo[newfd].username, 0, 256);
                        strcmp(send_buf,"Please enter your username:");
                        ...
                        while(1)
                        {
                            nbytes = read(tmp_i, data_buf, sizeof(data_buf)); //获取用户名
                            if (0 > nbytes)
                            {
                                perror("recv() error:");
                                exit(1);
                            ...
                            {
                                printf("The username can not be none!\n");
                            }
                            else                                               //保存用户名
                            {
                                strcat(clientinfo[tmp_i].username, data_buf);
                                clientinfo[tmp_i].username[strlen(clientinfo[tmp_i].username)-1] = '\0';
                                break;
                            }
                        }
                        clientinfo[tmp_i].first = 0;
							...
							else if(nbytes == 0 || (0 == strcmp(data_buf, "/quit")))
							{
								...
								clientinfo[tmp_i].online = 0;
							}

							else if(data_buf[0] == '/')
							{
							    if(0 == strcmp(data_buf, "/help"))
                                {
                                        strcat(send_buf,"Help message:\n");
                                        strcat(send_buf, "  /quit: exit\n");
                                        strcat(send_buf, "  /who: show online user\n");
                                        strcat(send_buf, "  /send: send msg to someone\n");
                                }
                                else if(0 == strcmp(data_buf, "/who"))
                                {
                                    strcat(send_buf,"Online user:\n");
                                    for (tmp_j = 0; tmp_j < BACKLOG; tmp_j++)
                                    {
                                        if ((0 != strcmp(clientinfo[tmp_j].username, "")) && clientinfo[newfd].online == 1)
                                        {
                                ...
                                else if( strstr(data_buf, "/send") != NULL)
                                {
                                    char *p;
                                    p = strtok(data_buf, " ");
                                    p = strtok(NULL, " ");
                                        for (tmp_j = 0; tmp_j < BACKLOG; tmp_j++)
                                        {
                                            if ((0 == strcmp(clientinfo[tmp_j].username, p))
                                                && clientinfo[newfd].online == 1)
                                            {
                                                strcat(send_buf, clientinfo[tmp_i].username);
                                                strcat(send_buf, " say to ");
                                                strcat(send_buf, clientinfo[tmp_j].username);
                                                strcat(send_buf, ": ");
                                                while(p != NULL)
                                                {
